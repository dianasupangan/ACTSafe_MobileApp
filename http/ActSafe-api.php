...
if(isset($_POST["state"])){
    ...
    //Scanned Device
        ...
        $get_duplicates = "INSERT INTO temp_scanned_devices SELECT id_number, uuid, date FROM scanned_devices GROUP BY id_number, uuid, date HAVING count(*) > 1;";
        mysqli_query($db,$get_duplicates);

        $delete_duplicates = "DELETE a FROM scanned_devices a INNER JOIN temp_scanned_devices b ON a.id_number = b.id_number AND a.uuid = b.uuid;";
        mysqli_query($db,$delete_duplicates);

        $insert_unique = "INSERT INTO scanned_devices SELECT id_number, uuid, date FROM temp_scanned_devices;";
        mysqli_query($db,$insert_unique);

        $delete_all_temp = "TRUNCATE `temp_scanned_devices`;";
        mysqli_query($db,$delete_all_temp);
